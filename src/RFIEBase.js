...
export default class RFIEBase extends React.Component {
  ...
  selectInputText = (element) => {
    // List of input types that support element.setSelectionRange
    ...
    // And if the element type doesn't support it we quit
    if(element.localName === 'input' && !supportedInput.includes(element.type)){
      return false;
    }
    ...
    makeClassString = (additionalClass = null) => {
      var classNames = [];
      if (!this.state.value && !this.state.editing) classNames.push(this.props.classPlaceholder);
